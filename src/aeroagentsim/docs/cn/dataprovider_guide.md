## AeroAgentSim 数据提供者框架文档

本文档解释了 AeroAgentSim 中的数据提供者框架，重点介绍如何将外部数据源集成到仿真环境中，以创建更加真实和动态的场景。

### 1. 概述

AeroAgentSim 中的数据提供者框架使外部数据源（如天气、交通和事故）能够集成到仿真环境中。这允许创建更加真实和动态的场景，代理可以对不断变化的环境条件做出反应。

*   **`DataProvider`:** 外部数据提供者的抽象基类。它定义了在仿真环境中加载数据和触发事件的通用接口。

*   **`DataIntegration`:** 数据集成模块的抽象基类。它充当 DataProviders 和仿真之间的桥梁，处理注册、配置和事件订阅逻辑。

*   **具体实现:** AeroAgentSim 包含几个 DataProviders 的具体实现，如 `WeatherDataProvider`、`TrafficDataProvider` 和 `AccidentDataProvider`。
...
AeroAgentSim 中的数据提供者框架使外部数据源能够集成到仿真环境中，从而创建更加真实和动态的场景。通过使用提供的数据提供者或创建自定义数据提供者，您可以使用真实世界的数据增强您的仿真，并创建更加复杂和真实的场景。
