## AeroAgentSim 代理开发者文档

本文档提供了理解和扩展 AeroAgentSim 框架中 `aeroagentsim.core.agent.Agent` 类的指导。它解释了核心概念、如何定义自定义代理类型以及如何实现它们的行为。
...
*   **目的：** 组件（`aeroagentsim.core.Component`）代表代理的功能部分（例如，'MobilitySystem'、'CPUEngine'、'CameraSensor'）。代理将任务执行委托给其组件。
*   **用法：**
...
*   **`Task` 对象：** 代表具有名称、状态、开始/结束时间、关联工作流等属性的工作单元。特定任务类型（例如，`MoveToTask`、`ComputeTask`）继承自 `aeroagentsim.core.Task`。
*   **`execute_task(...)` 方法：**
    *   这是代理启动工作的主要方式。
...
2.  **实现 `__init__`：**
    ...
    *   （可选）使用 `self.add_possessing_object(...)` 添加拥有的对象。
    ```python
    from aeroagentsim.component.mobility import MobilityComponent # 示例组件
    from aeroagentsim.resource import ChargingStation # 示例拥有的对象
...
通过遵循这些指南，您可以在 AeroAgentSim 框架内有效地创建多样化和能力强大的代理。参考 `Agent` 源代码和特定代理示例（如 `DroneAgent`）以获取具体的实现细节。
...
AeroAgentSim 使用 Python 的 `logging` 模块，支持以下日志级别：
- DEBUG：详细的调试信息
- INFO：一般信息，记录正常运行状态
- WARNING：警告信息，表示潜在问题
- ERROR：错误信息，表示严重问题
