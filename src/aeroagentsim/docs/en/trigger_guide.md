# AeroAgentSim Trigger System Guide

The AeroAgentSim trigger system provides a flexible way to react to various conditions and events within the simulation. Triggers are fundamental components for driving `WorkflowStatusMachine` transitions and automating agent behavior. This guide explains how to define and use different types of triggers.
...
AeroAgentSim provides several built-in trigger types:
...
The following operators (`aeroagentsim.core.enums.TriggerOperator`) are available for `EventTrigger` and `StateTrigger`:
