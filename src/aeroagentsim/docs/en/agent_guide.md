## AeroAgentSim Agent Developer Documentation

This document provides guidance on understanding and extending the `aeroagentsim.core.agent.Agent` class within the AeroAgentSim framework. It explains the core concepts, how to define custom agent types, and how to implement their behavior.
...
*   **Purpose:** Components (`aeroagentsim.core.Component`) represent the functional parts of an agent (e.g., 'MobilitySystem', 'CPUEngine', 'CameraSensor'). Agents delegate task execution to their components.
*   **Usage:**
...
*   **`Task` Object:** Represents a unit of work with properties like name, status, start/end times, associated workflow, etc. Specific task types (e.g., `MoveToTask`, `ComputeTask`) inherit from `aeroagentsim.core.Task`.
*   **`execute_task(...)` Method:**
...
1.  **Define the Class:**
    ...
    *   (Recommended) Create a companion metaclass inheriting from `AgentMeta` OR use the `@Agent.register_state_template` decorator to register type-specific states.
    ```python
    from aeroagentsim.core.agent import Agent, AgentMeta, StateTemplate
...
2.  **Implement `__init__`:**
    ...
    *   (Optional) Add possessed objects using `self.add_possessing_object(...)`.
    ```python
    from aeroagentsim.component.mobility import MobilityComponent # Example component
    from aeroagentsim.resource import ChargingStation # Example possessed object
...
By following these guidelines, you can effectively create diverse and capable agents within the AeroAgentSim framework. Refer to the `Agent` source code and specific agent examples (like `DroneAgent`) for concrete implementation details.
...
AeroAgentSim uses Python's `logging` module, supporting the following log levels:
