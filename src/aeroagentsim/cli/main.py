...
def export_docs(output_dir=None, format_type='markdown'):
    """
    Export AeroAgentSim documentation to the specified directory.

    Args:
        output_dir (str): The directory to export documentation to. Defaults to './aeroagentsim_docs'.
...
def export_markdown_docs(output_path):
    ...
    try:
        ...
        try:
            ...
            spec = importlib.util.find_spec('aeroagentsim') or importlib.util.find_spec('airfogsim')
        ...
        if docs_path is None:
            try:
                import aeroagentsim
                if hasattr(aeroagentsim, '__file__') and aeroagentsim.__file__ is not None:
                    package_path = Path(aeroagentsim.__file__).parent
                    potential_docs_path = package_path / 'docs'
                    if potential_docs_path.exists():
                        docs_path = potential_docs_path
            except Exception:
        ...
        if docs_path is None:
            current_dir = Path.cwd()
            potential_paths = [
                current_dir / 'src' / 'aeroagentsim' / 'docs',
                current_dir / 'aeroagentsim' / 'docs',
...
def create_index_file(output_dir):
    ...
    """
    index_content = """# AeroAgentSim Documentation
...
def run_examples(example_names=None):
    ...
    try:
        ...
        if examples_path is None:
            current_dir = Path.cwd()
            potential_paths = [
                current_dir / 'src' / 'aeroagentsim' / 'examples',
                current_dir / 'aeroagentsim' / 'examples',
...
def show_class_info(class_type=None, find_params=None):
    """
    Show information about AeroAgentSim classes.
    ...
    """
    try:
        from aeroagentsim.helper.class_finder import main as class_finder_main
...
def parse_args(args=None):
    ...
    """
    parser = argparse.ArgumentParser(description='AeroAgentSim Command Line Interface')
    subparsers = parser.add_subparsers(dest='command', help='Command to execute')

    # Docs command
    docs_parser = subparsers.add_parser('docs', help='Export documentation')
    docs_parser.add_argument('--output-dir', type=str, default='./aeroagentsim_docs',
...
def main():
    """
    Main entry point for the AeroAgentSim CLI.
