...
"""
import uuid
from aeroagentsim.core.enums import TriggerOperator, TriggerType
...
class Trigger:
    ...
    def _trigger(self, context: Dict[str, Any] = None):
        ...
        # 单次触发器默认触发后停用；周期触发器在达到最大次数后停用
        if self.has_reached_max() or not self.not_to_deactive:
            self._schedule_deactivation()
        return True
...
class TimeTrigger(Trigger):
    ...
    def __init__(self, env, trigger_time: Optional[float] = None,
        ...
        self.not_to_deactive = bool(self.interval is not None or self.cron_expr is not None)
