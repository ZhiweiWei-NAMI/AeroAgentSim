...
class ComponentManager:
    ...
    def _discover_component_classes(self):
        """自动发现并注册组件类"""
        # 获取 aeroagentsim.component 包的路径
        try:
            import aeroagentsim.component as component_pkg
            ...
            for _, module_name, is_pkg in pkgutil.iter_modules([str(component_path)]):
                ...
                try:
                    # 导入模块
                    module = importlib.import_module(f"aeroagentsim.component.{module_name}")
            ...
            logger.error("Error: 无法导入 aeroagentsim.component 包")
