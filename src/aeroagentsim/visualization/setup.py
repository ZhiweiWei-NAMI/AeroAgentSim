...
import aeroagentsim.agent as agent_pkg
...
from aeroagentsim.workflow.charging import create_charging_workflow
from aeroagentsim.workflow.image_processing import create_image_processing_workflow
from aeroagentsim.workflow.logistics import create_logistics_workflow
...
def setup_airspaces(env: PausableEnvironment, airspaces: List[Dict[str, Any]], log_event: Callable = None) -> None:
    ...
    if not callable(create_airspace):
        _emit_log(
            ...
            "AirspaceManager",
            (
                "当前运行时未暴露 create_airspace，跳过配置中的空域注入。"
                if airspaces
                else "当前运行时未暴露 create_airspace，跳过默认空域注入。"
            ),
            "warning",
        )
        return
...
def setup_frequencies(env: PausableEnvironment, frequencies: List[Dict[str, Any]], log_event: Callable = None) -> None:
    ...
    if not callable(create_frequency):
        _emit_log(
            ...
            "FrequencyManager",
            (
                "当前运行时未暴露 create_frequency，跳过配置中的频率注入。"
                if frequencies
                else "当前运行时未暴露 create_frequency，跳过默认频率注入。"
            ),
            "warning",
        )
        return
