...
class TaskManager:
    ...
    def __init__(self, env):
        ...
        # 注册事件
        self.env.event_registry.register_event(self.id, 'task_recommended')
        self.env.event_registry.register_event(self.id, 'task_selected')
    ...
    def register_task_class(self, task_class: Type[Task]) -> None:
        ...
        print(f"时间 {self.env.now}: 任务管理器注册任务类 {task_name}")
    ...
    def find_compatible_tasks(self, 
        ...
        if not agent_components:
            print(f"时间 {self.env.now}: 代理 {agent.id} 没有组件，无法执行任务")
            return []
    ...
    def create_task(self, 
        ...
        if not task_class:
            print(f"时间 {self.env.now}: 未找到任务类 {task_class_name}")
            return None
        
        components = agent.get_component_names()
        # 检查组件是否存在
        if component_name not in components:
            print(f"时间 {self.env.now}: 代理 {agent.id} 没有组件 {component_name}")
            return None
        ...
        try:
            ...
            print(f"时间 {self.env.now}: 创建任务 {task_class_name} 失败: {str(e)}")
            import traceback
            traceback.print_exc()
            return None
