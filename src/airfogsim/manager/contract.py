...
class ContractManager:
    ...
    def _register_events(self):
        ...
        for event in events:
            self.env.event_registry.register_event(self.manager_id, event)
    ...
    def create_contract(self, issuer_agent_id, task_info, reward, penalty=0, 
        ...
        if reward > 0 and isinstance(reward, (int, float)):
            if self.agent_balances[issuer_agent_id] < reward:
                print(f"时间 {self.env.now}: 代理 {issuer_agent_id} 余额不足，无法创建合约")
                return None
            # 冻结余额
            self.agent_balances[issuer_agent_id] -= reward
            self.frozen_balances[issuer_agent_id] += reward
            print(f"时间 {self.env.now}: 代理 {issuer_agent_id} 冻结 {reward} 单位余额用于合约")
        ...
        print(f"时间 {self.env.now}: 合约 {contract_id} 已创建")
        return contract_id
    ...
    def accept_contract(self, contract_id, executor_agent_id):
        ...
        if contract_id not in self.contracts:
            print(f"时间 {self.env.now}: 合约 {contract_id} 不存在")
            return False
        ...
        try:
            ...
            if not issuer_agent:
                print(f"时间 {self.env.now}: 无法找到合约发起者 {contract['issuer_agent_id']}")
                return False
            executor_agent = self.env.get_agent(executor_agent_id)
            if not executor_agent:
                print(f"时间 {self.env.now}: 无法找到合约执行者 {executor_agent_id}")
                return False
            # 创建合约工作流
            ...
            if hasattr(self.env, 'workflow_manager'):
                ...
                print(f"时间 {self.env.now}: 环境中没有工作流管理器，无法注册工作流")
                return False
            ...
            print(f"时间 {self.env.now}: 合约 {contract_id} 创建了工作流 {workflow.id}")
            
        except Exception as e:
            print(f"时间 {self.env.now}: 创建合约工作流失败: {str(e)}")
        ...
        print(f"时间 {self.env.now}: 合约 {contract_id} 已被 {executor_agent_id} 接受")
        return True
    ...
    def _on_contract_completed(self, contract_id, success=True, verification_result=None):
        ...
        if contract['status'] != ContractStatus.ACTIVE.name:
            print(f"时间 {self.env.now}: 合约 {contract_id} 状态为 {contract['status']}，无法完成")
            return False
        ...
        if contract['reward'] > 0 and isinstance(contract['reward'], (int, float)):
            if self.frozen_balances[issuer_agent_id] >= contract['reward']:
                ...
                print(f"时间 {self.env.now}: 代理 {issuer_agent_id} 解冻 {contract['reward']} 单位余额（取消合约）")
        ...
        if contract['executor_agent_id'] and contract['penalty'] > 0:
            ...
            if self.frozen_balances[executor_agent_id] >= contract['penalty']:
                self.frozen_balances[executor_agent_id] -= contract['penalty']
                self.agent_balances[executor_agent_id] += contract['penalty']
                print(f"时间 {self.env.now}: 代理 {executor_agent_id} 解冻 {contract['penalty']} 单位余额（取消合约）")
        ...
        print(f"时间 {self.env.now}: 合约 {contract_id} 已{'成功' if success else '失败'}完成")
        return True
    ...
    def cancel_contract(self, contract_id, reason=None):
        ...
        if contract['status'] not in [ContractStatus.PENDING.name, ContractStatus.ACTIVE.name]:
            print(f"时间 {self.env.now}: 合约 {contract_id} 状态为 {contract['status']}，无法取消")
            return False
        ...
        print(f"时间 {self.env.now}: 合约 {contract_id} 已取消，原因: {reason}")
        return True
    ...
    def execute_transaction(self, from_agent_id, to_agent_id, amount, reason=None):
        ...
        print(f"时间 {self.env.now}: 交易执行成功: {from_agent_id} -> {to_agent_id}, 金额: {amount}, 原因: {reason}")
        return True
