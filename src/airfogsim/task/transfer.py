...
class FileTransferTask(Task):
    ...
    PRODUCED_STATES = ['trans_target_agent_id', 'transfer_progress', 
                       'transfer_speed','is_transmitting']
    ...
    def __init__(self, env, agent, component_name: str, task_name: str,
        ...
        if not self.file_manager.get_file_owner(self.file_id) != agent.id:
        ...
        self.agent.set_state('transfer_progress', 0.0)
        self.agent.set_state('transfer_speed', 0.0)
    ...
    def _update_task_state(self, performance_metrics: Dict):
        ...
        # 更新代理状态
        self.agent.set_state('transfer_progress', self.transfer_progress)
        self.agent.set_state('transfer_speed', self.transfer_speed)
