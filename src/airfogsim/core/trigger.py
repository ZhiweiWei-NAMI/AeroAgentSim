        ...
        if self.operator == TriggerOperator.EQUALS:
            ...
            if callable(self.target_value):
                try:
                    ...
                    print(f"时间 {self.env.now}: 触发器 {self.name} 自定义条件函数执行错误: {e}")
                    return False
            else:
                print(f"时间 {self.env.now}: 触发器 {self.name} 使用CUSTOM运算符但target_value不是可调用对象")
                return False
        else:
...
class StateTrigger(Trigger):
    ...
    def _monitor(self):
        ...
        try:
            ...
            if not agent:
                print(f"时间 {self.env.now}: 触发器 {self.name} 找不到代理 {self.agent_id}")
                return
