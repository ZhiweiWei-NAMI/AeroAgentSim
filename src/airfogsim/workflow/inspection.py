...
class InspectionWorkflow(Workflow, metaclass=InspectionWorkflowMeta):
    ...
    def __init__(self, env, name, owner, timeout=None, proof_id=None, 
        ...
        super().__init__(
            ...
            owner=owner,
            timeout=timeout,
            proof_id=proof_id or f"inspection_proof_{uuid.uuid4().hex}",
    ...
    def _setup_transitions(self):
        ...
        for i, point in enumerate(self.inspection_points):
            ...
            # 添加回调以触发 waypoint_completed 事件
            self._add_waypoint_callback(i)
    ...
    def _add_waypoint_callback(self, point_index):
        """添加检查点完成时的回调"""
        def on_waypoint_reached(context):
            # 触发检查点完成事件
        ...
        # 获取当前状态的所有触发器
        current_state = f'inspecting_point_{point_index+1}'
        transitions = self.status_machine._get_current_transitions()
        
        # 为相关触发器添加回调
        for trigger, next_state in transitions:
            if trigger.source_id == self.proof_id and trigger.event_name == 'proof_updated':
                trigger.add_callback(on_waypoint_reached)
                break
...
def create_inspection_workflow(env, agent, inspection_points):
    ...
    proof_id = f"inspection_{uuid.uuid4().hex}"
