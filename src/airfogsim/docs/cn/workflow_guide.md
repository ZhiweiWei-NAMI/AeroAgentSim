## AirFogSim 工作流-代理-任务框架文档

本文档解释了 AirFogSim 中的工作流-代理-任务框架，重点关注 `Workflow` 和 `WorkflowStatusMachine` 类，以及它们如何通过可重用组件的组合来实现复杂任务场景的建模。

### 1. 概述

AirFogSim 的核心是工作流-代理-任务框架，它通过可重用组件的组合实现复杂任务场景的建模。任务（Tasks）代表原子执行单元，而工作流（Workflows）定义更高级别的过程、目标或操作序列。
