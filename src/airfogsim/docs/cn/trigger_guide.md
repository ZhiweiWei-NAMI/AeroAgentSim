...
AirFogSim 触发器系统提供了一种灵活的方式来对仿真中的各种条件和事件做出反应。触发器是驱动 `WorkflowStatusMachine` 转换和自动化代理行为的基本组件。本指南解释了如何定义和使用不同类型的触发器。
...
`EventTrigger` 和 `StateTrigger` 可以使用以下运算符（`airfogsim.core.enums.TriggerOperator`）：
