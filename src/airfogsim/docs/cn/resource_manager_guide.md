...
本文档解释了 AirFogSim 中的资源管理系统，重点关注 `Resource` 基类、通用 `ResourceManager<R>` 基类以及特定实现，如 `LandingResource` 和 `LandingManager`。
...
AirFogSim 为不同类型的资源提供了几个专门的资源管理器：
