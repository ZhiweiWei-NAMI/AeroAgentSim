...
def parseD3QNTrainArgs():
    ...
    parser.add_argument('--eps_end', type=float, default=0.05)  # 最低探索系数
    parser.add_argument('--eps_dec', type=float, default=1e-3)  # 探索系数衰减率
...
def parseD3QNDimArgs():
    ...
    dim_mission=13
    # [type, accuracy,candidate,x,y,z,is_approaching,in_threshold_predict]
    dim_sensor=8
    max_sensors=6
    m_u=15
    m_v=100
    ...
    # dim_actions=delay+max_sensors * (m_v + m_u)
    dim_actions = 21
...
class D3QN_Train_AlgorithmModule(BaseAlgorithmModule):
    ...
    def initialize(self, env: AirFogSimEnv, config={}, last_episode=None,final=False):
        ...
        # self.rewardScheduler.setModel(env, 'REWARD','_mission_duration_sum * _mission_accuracy ')
        # self.rewardScheduler.setModel(env, 'PUNISH', '- 5*_mission_duration_sum * _mission_accuracy')
    ...
    def _encode_sensor_states(self, sensor_states, max_sensor_node_num, node_sensor_num, dim_state,grouping=True):
        # [node_id,node_type,id,type, accuracy,candidate,distance,x,y,z,is_approaching,in_threshold_predict]
        ...
        if grouping is False:
            ...
            for sensor_state in sensor_states:
                ...
                processed_state = [type,accuracy,candidate,position_x,position_y,position_z,is_approaching,in_threshold_predict]
    ...
    def saveModel(self, episode,final):
        self.D3QN_env.saveModel(episode,final)
    ...
    def scheduleMission(self, env: AirFogSimEnv):
        ...
        for mission_profile in new_missions_profile:
            ...
            encode_mask=encode_mask[:self.D3QN_dim_args.dim_actions-1]
            encode_mask = np.insert(encode_mask[:self.D3QN_dim_args.dim_actions], 0, delay_times<max_delay_times) # 允许relay动作
            ...
            if action_index==0:
                exp_id = f'{mission_id}_delay_{delay_times}'
                self.ta_idx[exp_id]=action_index
                env.addMissionTaIdx(action_index)
                delay_times+=1
                mission_profile['delay_times']=delay_times
                reward=-delay_times*0.4+max(0,10-valid_sensor_num)
                # set state buffer
                if self.last_mission_id is not None:
                    self.ta_buffer.setNextState(self.last_mission_id, combined_states, mask_flat, False)
                self.ta_buffer.add(exp_id, combined_states,mask_flat,action_index,reward)
                self.last_mission_id = exp_id
                self.delay_exp_ids.append(exp_id)
            ...
            if appointed_node_id is not None and appointed_sensor_id is not None:
                ...
                pre_reward = min(0, valid_sensor_num - 5)
                # set state buffer
                if self.last_mission_id is not None:
                    self.ta_buffer.setNextState(self.last_mission_id, combined_states, mask_flat, False)
