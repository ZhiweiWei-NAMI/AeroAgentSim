...
class WorstAlgorithmModule(BaseAlgorithmModule):
    ...
    scheduleMission: 
        ...
        Sensor: Select the sensor farthest to PoI from the idle sensors with accuracy higher than required, and in distance threshold(Distance First).
                Assigned to UAV, select the sensor with lowest accuracy from the idle sensors with accuracy higher than required.
    ...
    scheduleTraffic: UAV: Go to farthest mission position in random.

    '''

    def __init__(self):
        super().__init__()
        self.algorithm_module_tag="Worst"
    ...
    def scheduleMission(self, env: AirFogSimEnv):
        ...
        for mission_profile in new_missions_profile:
            ...
            appointed_node_id, appointed_sensor_id, appointed_sensor_accuracy = self.sensorScheduler.getFarthestIdleSensorInNodes(
    ...
    def scheduleTraffic(self, env: AirFogSimEnv):
        ...
        for UAV_id, UAV_info in UAVs_info.items():
            ...
            target_position = self.missionScheduler.getFarthestMissionPosition(env, UAV_id, current_position)
