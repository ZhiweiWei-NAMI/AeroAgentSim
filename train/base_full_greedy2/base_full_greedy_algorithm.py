...
class FullGreedyAlgorithmModule(BaseAlgorithmModule):
    ...
    scheduleMission: 
        Mission: Missions assigned to both vehicles and UAVs.
        Sensor: Select the sensor closest to PoI from the idle sensors with accuracy higher than required, and in distance threshold.
    ...
    scheduleTraffic: UAV: Move to the nearest mission position. If there is no mission allocated to UAV, move to cluster center.
    '''
    ...
    def scheduleStep(self, env: AirFogSimEnv):
        ...
        #     print('sensing_data:', sensing_data)
    ...
    def scheduleMission(self, env: AirFogSimEnv):
        ...
        for mission_profile in new_missions_profile:
            ...
            appointed_node_id, appointed_sensor_id, appointed_sensor_accuracy = self.sensorScheduler.getSuitableIdleSensorInNodes(
                env, mission_sensor_type, mission_accuracy, sensing_position, node_infos, excluded_sensor_ids,delay=False)
    ...
    def scheduleTraffic(self, env: AirFogSimEnv):
        ...
        for UAV_id, UAV_info in UAVs_info.items():
            ...
            if next_mission_position is None:
                mission_states=self.algorithmScheduler.getBeforeTransMissionStates(env,current_position,observe_threshold)
