...
from airfogsim.algorithm.crowdsensing.MAPPO_share.MAPPO_env import MAPPO_Env
...
if cuda_num > 0:
    ...
    MAPPO_device = f"cuda:{cuda_list[cuda_num - 4]}"
else:
    MAPPO_device = "cpu"
...
def parseMAPPOTrainArgs():
    ...
    parser.add_argument('--learning_rate', type=float, default=3e-4)  # 学习率
    parser.add_argument('--learning_rate_min', type=float, default=1e-5)  # 最低学习率
    parser.add_argument('--learning_rate_gamma', type=float, default=0.99)  # 学习率衰减速率
    parser.add_argument('--step_size', type=int, default=100)  # 学习率衰减频率
...
class MAPPO_Train_AlgorithmModule(BaseAlgorithmModule):
    ...
    def __init__(self):
        super().__init__()
        self.algorithm_module_tag = "MAPPO_share_Train"
