...
class RandomAlgorithmModule(BaseAlgorithmModule):
    ...
    scheduleMission: 
        ...
        Sensor: Select the sensor randomly from the idle sensors with accuracy higher than required, and in distance threshold.
    ...
    scheduleTraffic: UAV: Move to the mission position randomly. If there is no mission allocated to UAV, movement is random.
    '''

    def __init__(self):
        super().__init__()
        self.algorithm_module_tag="Random"
    ...
    def scheduleMission(self, env: AirFogSimEnv):
        ...
        for mission_profile in new_missions_profile:
            ...
            appointed_node_id, appointed_sensor_id, appointed_sensor_accuracy = self.sensorScheduler.getRandomIdleSensorInNodes(
                env, mission_sensor_type, mission_accuracy, node_infos, excluded_sensor_ids)
            ...
            if appointed_node_id != None and appointed_sensor_id != None:
                ...
                if node_type=='U':
                    self.trafficScheduler.addUAVRoute(env, mission_profile['mission_id'],appointed_node_id, mission_profile['mission_routes'][0],mission_profile['mission_duration'][0]*UAV_time_discount,mission_profile['mission_arrival_time']+ mission_profile['mission_deadline'])
    ...
    def scheduleTraffic(self, env: AirFogSimEnv):
        """The UAV traffic scheduling logic. Move to the next random mission sensing or task position. If there is no mission allocated to UAV, movement is random.
        ...
        for UAV_id, UAV_info in UAVs_info.items():
            ...
            # target_position = self.trafficScheduler.getNextPositionOfUAV(env, UAV_id,is_random=True)
