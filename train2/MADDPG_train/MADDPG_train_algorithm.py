...
if cuda_num > 0:
    ...
    MADDPG_device = f"cuda:{cuda_list[cuda_num - 3]}"
else:
    MADDPG_device = "cpu"

print('device: ',MADDPG_device)
...
def parseMADDPGTrainArgs():
    ...
    parser.add_argument('--var', type=float, default=0.5)  # 动作探索随机噪声
...
class MADDPG_Train_AlgorithmModule(BaseAlgorithmModule):
    """
    ...
    def __init__(self):
        super().__init__()
        self.algorithm_module_tag = "MADDPG_Train"
    ...
    def initialize(self, env: AirFogSimEnv, config={}, last_episode=None,final= False):
        ...
        if last_episode is not None and last_episode > 0:
            self.MADDPG_env.loadModel(last_episode,final)
    ...
    def train(self, env: AirFogSimEnv):
        self.updatePPExperience(env) #
        a_loss,c_loss=self.MADDPG_env.train()
        return a_loss,c_loss

    def saveModel(self, episode,final):
        self.MADDPG_env.saveModel(episode,final)
