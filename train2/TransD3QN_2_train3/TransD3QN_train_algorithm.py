...
from airfogsim.algorithm.crowdsensing.TransD3QN2.TransD3QN_env import TransD3QN_Env
...
def parseTransD3QNTrainArgs():
    parser = argparse.ArgumentParser(description='TransD3QN2 train arguments')
...
def parseTransD3QNDimArgs():
    ...
    parser.add_argument('--dim_actions', type=float, default=dim_actions)  # Dimension of hidden layer
...
class TransD3QN_Train_AlgorithmModule(BaseAlgorithmModule):
    ...
    class TaskAllocationReplayBuffer(BaseReplayBuffer):
        ...
        def __expToFlattenArray(self, exp):
            ...
            return np.array(mission_state), np.array(sensor_state), np.array(
                sensor_mask), np.array(action), np.array(reward), np.array(next_mission_state), np.array(
                next_sensor_state), np.array(next_sensor_mask), np.array(done)

        def add(self, exp_id,mission_state, sensor_state, sensor_mask, action, reward=None,
                next_mission_state=None, next_sensor_state=None, next_sensor_mask=None,
                done=None):
            self.buffer[exp_id] = {'mission_state': mission_state,
                                   'sensor_state': sensor_state, 'sensor_mask': sensor_mask, 'action': action,
                                   'reward': reward,
                                   'next_mission_state': next_mission_state,
                                   'next_sensor_state': next_sensor_state, 'next_sensor_mask': next_sensor_mask,
                                   'done': done}

        def setNextState(self, exp_id, next_mission_state, next_sensor_state, next_sensor_mask, done):
        ...
        def completeAndPopExperience(self, exp_id, reward):
            if exp_id not in self.buffer:
                return None
            if self.buffer[exp_id]['next_mission_state'] is None:
                return None
    ...
    def _encode_node_states(self, node_states, max_node_num, dim_state):
        ...
        for node_state in node_states:
            ...
            if node_state[1] == 'U':
                position_z = (node_state[6] - self.min_position_z) / (self.max_position_z - self.min_position_z) if (
                                                                                                                                self.max_position_z - self.min_position_z) > 0 else 1
            else:
                position_z = 0
    ...
    def _encode_mission_states(self, mission_states, max_mission_num, current_time, dim_state, delay_times,
                               max_delay_times, valid_sensor_num):
        ...
        max_valid_sensor_num = self.TransD3QN_dim_args.dim_actions
        ...
        for mission_state in mission_states:
            ...
            position_z = (mission_state[8] - self.min_position_z) / (self.max_position_z - self.min_position_z) if (
                                                                                                                               self.max_position_z - self.min_position_z) > 0 else 0
            distance_threshold = mission_state[9] / (self.max_position_x - self.min_position_x)

            state = [norm_cur_time, sensor_type, accuracy, return_size, arrival_time, TTL, duration, position_x,
                     position_y,
    ...
    def _encode_sensor_states(self, sensor_states, max_sensor_num, max_sensor_node_num, node_sensor_num, dim_state,
                              grouping=True):
        ...
        if grouping is False:
            ...
            for sensor_state in sensor_states:
                ...
                if node_type == 'U':
                    position_z = (sensor_state[9] - self.min_position_z) / (
                                self.max_position_z - self.min_position_z) if (
                                                                                          self.max_position_z - self.min_position_z) > 0 else 0
                else:
                    position_z = 0
                ...
                processed_state = [type, accuracy, candidate, position_x, position_y, position_z, is_approaching,
                                   in_threshold_predict, node_speed]
    ...
    def scheduleMission(self, env: AirFogSimEnv):
        ...
        for mission_profile in new_missions_profile:
            ...
            encode_mission_state = self._encode_mission_states(mission_states, 1, cur_time,
                                                               self.TransD3QN_dim_args.dim_mission, delay_times,
                                                               max_delay_times, valid_sensor_num)
            ...
            is_random, max_q_value, action_index = self.TransD3QN_env.takeAction(encode_mission_state,
            ...
            if appointed_node_id is not None and appointed_sensor_id is not None:
                ...
                if appointed_node_type == 'U':
                    self.trafficScheduler.addUAVRoute(env, mission_profile['mission_id'], appointed_node_id,
                                                      ...
                                                      mission_profile['mission_duration'][0],
                ...
                if valid_sensor_num > 1:
                    ...
                    if self.last_mission_id is not None:
                        self.ta_buffer.setNextState(self.last_mission_id, encode_mission_state,
                                                    encode_sensor_states, encode_mask, False)
                    self.ta_buffer.add(mission_id, encode_mission_state, encode_sensor_states,
                                       encode_mask,action_index)
