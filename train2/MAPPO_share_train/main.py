...
from train.MAPPO_share_train.MAPPO_share_train_algorithm import MAPPO_Train_AlgorithmModule
...
for episode in range(last_episode + 1, max_episode + 1):
    ...
    a_loss, c_loss = algorithm_module.train(env)  # 训练模型，交互结束后再训练
    # 存储loss
    ...
    algorithm_module.saveModel(episode, final=True,succ_ratio=succ_ratio)  # 保存模型
    if episode % checkpoint == 0:
        algorithm_module.saveModel(episode, final=False,succ_ratio=succ_ratio)
    ...
    if episode > 1:
        ...
        for i in range(len(actor_step_loss)):
            actor_acc_reward = evaluation_module.getUAVAccReward(i)
            actor_avg_reward = evaluation_module.getUAVAvgReward(i)
            actor_avg_loss = sum(actor_step_loss[i]) / len(actor_step_loss[i])
            writer.add_scalar(f"actor_acc_reward_{i}", actor_acc_reward, episode)
            writer.add_scalar(f"actor_avg_reward_{i}", actor_avg_reward, episode)
            writer.add_scalar(f"actor_loss_{i}", actor_avg_loss, episode)
