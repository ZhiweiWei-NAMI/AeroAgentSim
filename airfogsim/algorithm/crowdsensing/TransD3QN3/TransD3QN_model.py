...
class TransD3QN:
    ...
    def remember(self,node_state, mission_state, action,  reward,next_node_state, next_mission_state, done):
        self.memory.add(node_state, mission_state,action, reward,next_node_state,  next_mission_state, done)

    # 动作选择
    def take_action(self,node_state, mission_state):
        """
        Args:
            node_state: [1, dim_node]
            mission_state: [max_sensors,dim_mission]
        """
        # numpy[1, dim_node]-->[1, 1, dim_node]-->Tensor
        node_state = torch.tensor(node_state[np.newaxis, :], dtype=torch.float).to(self.device)
        # numpy[max_sensors,dim_mission]-->[1, max_sensors,dim_mission]-->Tensor
        mission_state = torch.tensor(mission_state[np.newaxis, :], dtype=torch.float).to(self.device)

        with torch.no_grad():
            # 获取当前状态下采取各动作的q值
            q_values = self.q_net( node_state, mission_state)
        ...
        # 如果大于贪婪系数就随机探索
            ...
            is_random = True
            action =random.randint(0, self.dim_actions-1)
    ...
    def update(self):
        if not self.memory.ready():
            return
        node_states,mission_states, actions, rewards, next_node_states, next_mission_states, dones = self.memory.sample(self.batch_size)

        # 当前状态
        # numpy[batch_size, 1, dim_node]-->[batch_size, 1, dim_node]-->Tensor
        node_states = torch.tensor(node_states, dtype=torch.float).to(self.device)
        # numpy[batch_size,max_sensors,dim_mission]-->Tensor[batch_size,max_sensors, dim_mission]
        ...
        with torch.no_grad():
            next_q_values = self.q_net.forward(next_node_states,next_mission_states)
            # [batch_size] -> [batch_size,index_size(1)]
            max_next_actions = torch.argmax(next_q_values, dim=1).unsqueeze(-1)
            next_q_targets = self.target_q_net.forward(next_node_states,next_mission_states)
            td_q_targets = rewards + self.gamma * next_q_targets.gather(1, max_next_actions) * (1 - dones.int())
        q_values = self.q_net(node_states,mission_states).gather(1, actions)
