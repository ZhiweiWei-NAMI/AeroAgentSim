...
class Net(nn.Module):
    def __init__(self, dim_args):
        ...
        self.dim_node = dim_args.dim_self_UAV
        self.dim_mission = dim_args.dim_todo_mission
        ...
        # 有两个隐含层
        self.fc1 = nn.Linear((self.max_sensors+1) * self.dim_model, self.dim_hiddens)
    ...
    def forward(self, node_state,mission_state):
        """
        Args:
            node_state: [batch_size,1, dim_node]
            mission_state: [batch_size,max_sensors,dim_mission]
        """
        batch_size = node_state.size(0)

        # Process neighbor task nodes and data
        node_state_emb = self.node_embedding(node_state)  # [batch_size, 1, dim_model]
        mission_state_emb = self.mission_embedding(mission_state)  # [batch_size, max_sensors, dim_model]

        # Combine all sequences
        combined_sequence = T.cat([
            node_state_emb,# node [batch_size, 1, dim_model]
            mission_state_emb,  # mission [batch_size, max_sensors, dim_model]
        ], dim=1)  # [batch_size,  1+ max_sensors, dim_model ]
        ...
        flatten_outputs = transformer_output.reshape(batch_size,-1)  # [batch_size, 1+max_sensors * dim_model ]
